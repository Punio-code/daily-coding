...
/*
  ...
  ▼修正ポイント
  「変数の値が～以上であるか」で判定する場合は「高い値から順番に」判定し、
  「変数の値が～以下であるか」で判定する場合は「低い値から順番に」判定すると漏れなく判定できる。（今回は前者で実装した）
  具体的には `age >= 65` → `age >= 13` → `age >= 6` → （elseで 0〜5歳）。
  これで 65歳以上が先にマッチするため、13歳以上の分岐に飲み込まれることがない。
*/
